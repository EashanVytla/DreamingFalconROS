...
class ReplayBuffer:
    def __init__(self, config):
        ...
        self.states = torch.zeros((self.capacity, config.force_model.state_dim), dtype=torch.float32, device=self.device).share_memory_()
    ...
    def add(self, state: torch.Tensor, action: torch.Tensor, dt: float):
        with self.lock:
            ...
            self.states[self.ptr.value, :] = norm_x_t
    ...
    def sample(self, batch_size: int, sequence_length: int):
        with self.lock:
            ...
            start_indices = np.random.randint(0, available_data - sequence_length + 1, size=(batch_size))
